...
def get_sampler(training_data, language_set, config):
    total_examples = 0
    if isinstance(training_data[language_set[0]], list):
        #Still at document level, need to flatten
        for k,v in training_data.items():
            flattened = [example for document in v for example in document]
            training_data[k] = flattened
    ...
    for k,v in training_data.items():
        ...
        exponentiated_probability = math.pow(language_probability, config['data_settings']['alpha'])
    ...
    return flattened_training_data, sampler
